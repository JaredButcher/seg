...
Decoys, unlike the above, **are** built (`content/weapons.ts`'s `active-decoy`,
`sim/weapons/decoy.ts`): a decoy runs at the flank speed of the boat that fired it, radiating that
boat's silhouette and noise, and a seeker has no way to tell it from the real hull until someone
pings it.
...
- 0 HP → destroyed. A louder transient than an ordinary hit (`hull-destroyed`, distinct from
  `hull-damage`) marks the moment, and the hull keeps making noise afterward — air escaping and
  metal groaning, continuous rather than decaying, for as long as it is on the map. It is still a
  reflector and a sonar contact, and now a legitimate one: a torpedo's seeker may lock onto a
  wreck and the fuze will detonate on it, where earlier revisions of this design had both ignore
  it. **The wreck sinks** rather than resting on the seabed forever (revised from Q24): it
  descends slowly and **despawns once it has sunk below the bottom of the map**, so a battlefield
  that used to accumulate confusion permanently now clears itself over the course of a match.
  For legibility, every wreck on the map is shown to **every player**, not gated on the sonar
  picture — grey rather than a team's colour, with a few bubbles drawn above it, on both the
  scope and the mini-map. A destroyed hull is no longer a threat, so there is nothing left for
  the fog of war to protect by hiding it.
