...
| C10 | Map authoring | **Procedurally generated dense cave systems**, replacing three authored maps. New document [14-map-generation.md](14-map-generation.md). | Terrain became the dominant system. Retires Q9 (map scaling — extents are now a generation parameter, so there is nothing to stretch) and Q15 (map count — unbounded). Moved map generation into M1, growing it to ~5 weeks, because acoustics and navigation both consume the generator's sector decomposition. |
| C11 | Terrain occlusion model | **Portal propagation over a precomputed sector graph**, not raycasting. Sound bends through openings; a relayed bearing points at the **portal**, not the source. | Makes dense terrain cheaper at runtime than sparse terrain would have been with raycasts (03 §5.2), and produces the game's richest mechanic — honest, learnable misdirection. Raises a new UI risk, Q41. |
...
| Q32 | **Map extents and the range scale.** 5000 m × 1200 m is a first guess, and every detection and weapon range in 05 hangs off it. (03 §9) | Prototype it in the M1 harness before the content tables are written. The aspect ratio (~4:1) and the ratio of detection range to map width are the numbers that matter, not the absolutes. Getting this wrong makes everything downstream wrong. | M1 | All content ranges, map authoring, camera |
...
| Q41 | **Do relayed bearings read as a puzzle or as broken sensors?** (03 §5.1, 08 §4) | The highest-risk consequence of C11. A bearing pointing at a cave mouth instead of a boat is either the game's best mechanic or a stream of bug reports, and presentation decides which. Prototype the UI at M2 against the ugly renderer; if players cannot learn it, fall back to marking relayed contacts as range-only with no bearing rather than showing a misleading one. | M1 (feel), M2 (UI) | TMA design, contact rendering |
...
| Q11 | **Is the closing map visible to players?** (06 §2.1) | **Yes**, drawn as hard lines with warning before it moves. Note it now closes horizontally *and upward from the seabed* — squeezing both teams into the shallow, loud water, which is a better pressure mechanic than a top-down ring. | M4 | Deathmatch pacing |
