...
| C12 | Terrain knowledge (was Q12) | ~~**Charted from match start**, drawn dim; sonar makes local geometry crisp.~~ **Reversed by C21.** | The navigational argument did not hold — the *server* pathfinds, and "no route" is already an explained refusal (08 §5) — and charting the map at tick zero solved the dominant system in the game before the first order. |
...
| C21 | Terrain knowledge (reverses C12) | **Players start uncharted and fill the map in by sonar.** A playing client is sent a `MapChart` — extents and scale, no obstacles, **no seed** — and terrain reaches spectators only. A square of rock confirmed by the server is charted for the rest of the match; a confirmed hostile hull is revealed whole and decays to a hollow last-known outline. Two thresholds: `detectionThreshold` to appear, `confirmationThreshold` to be committed to. | Restores the pitch — "build an acoustic picture of the map faster than the enemy builds one of you" — and closes a real leak: generation is pure and the client bundles it, so the seed *was* the map. Adds `match/vision.ts` (the chart, the contact book, the wire encoding), the `sonar` accent (09 §4), and a per-recipient chart watermark. The bandwidth of a 1 m picture at 10 Hz is the open cost, held down by send-once charting, a wire cap, ascending delta encoding, and half-decibel quantization, and **not yet measured**. See [ADR 0002](../docs/adr/0002-uncharted-terrain.md). |
...
| C22 | Active sonar shape | **A pulse is a loud transient, not a simulated wavefront.** `pingLevel` is a stat, a pulse rings down over 0.4 s through the existing `EmitState.transients` path, and nothing in the solver knows what a ping is. Active sonar is a *posture* — a switch pulsing every **1000 ms** — rather than a fire-once action. The expanding ring and the panned tone on the client are presentation. | Answers 03 §9.1's finding that a match opens on a screen with nothing on it: a stopped boat charts *zero* squares in two seconds and 2100–2700 with the switch on. Gets 03 §3's "bigger detection radius for a bigger self-broadcast radius" trade out of one `+8 dB` modifier rather than a rule, and leaves 03 §6's wavefront, return delay, and outline trace unbuilt and undamaged. Also the project's **first client command** (`match.setActiveSonar`), which settled the command shape: names a target, idempotent, unacknowledged. Measured asymmetry is severe — you are heard 4–6× further than you can see (03 §9.2) — and unretuned pending the balance harness. See [ADR 0003](../docs/adr/0003-active-sonar-is-a-transient.md). |
...
| Q32 | **Map extents and the range scale.** 8000 m × 3000 m base (medium), with a fixed 1000 m game depth, is the current constant set — and every detection and weapon range in 05 hangs off it. (03 §9) | Prototype it in the M1 harness before the content tables are written. The aspect ratio (~2.7:1) and the ratio of detection range to map width are the numbers that matter, not the absolutes. Getting this wrong makes everything downstream wrong. | M1 | All content ranges, map authoring, camera |
| Q33 | **Max pitch band.** ±25–35° per hull is asserted, untested. **Hulls only** — torpedo pitch limits were built, play-tested, and removed (05 §4), so this is no longer one balance problem with two halves. (04 §5) | Prototype. Too shallow and depth changes are unusably slow; too steep and boats stop reading as submarines. The "vertical evasion trivializes torpedoes" half of this question is settled rather than open: a weapon follows a climbing target as readily as a turning one, and what beats it is its turning circle. | M1 | Movement feel |
...
| Q24 | **Wreck lifetime?** (04 §8) | **Revised: not permanent.** A wreck sinks and despawns once it has sunk below the bottom of the map, rather than resting on the seabed for the rest of the match — a battlefield that clears itself rather than one that only ever accumulates. It stays a legitimate sonar contact and seeker target while it is still sinking, and is shown to every player, grey and unconditional on the sonar picture, for as long as it is there. | M3 | Sim, tracker |
