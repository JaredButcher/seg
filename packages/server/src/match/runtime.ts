...
export class MatchRuntime {
  ...
  tick(): boolean {
    ...
    this.current = {
      ...
      // Before the solve, and at the full 20 Hz rather than the 10 Hz acoustic rate: a pulse's
      // level is read from how long ago it fired, so firing it on the sim clock and sampling it
      // on the acoustic clock is what keeps the two rates independent. Nothing here depends on
      // whether this tick happens to be a solve tick.
      boats: this.pulse(tick),
    };
